...
def engine(json_dataset):
    ...
    for row in json_dataset:
        filename = os.path.join(OUTPUT_DIR_NAME, f'{valid_available_name(row)}.json')
...
def parse_attributes(json_object):
    str_attributes = json_object.get('attributes') or json_object.get('attributes-_hair._eyes._teeth._clothing._accessories._expression._strength._weakness')
